...
<p align="center">
  ...
  <a href="https://github.com/AnanyaP-WDW/AdTestPro/blob/main/LICENSE"><img src="https://img.shields.io/badge/license-MIT-blue" alt="License: MIT"></a>
  <img src="https://img.shields.io/badge/python-3.11-blue" alt="Python 3.11">
  <img src="https://img.shields.io/badge/tests-83%20offline-brightgreen" alt="83 offline tests">
...
questions, set the panel size, and run. `GET /ready` reports `{"ready": true}` when
...
|---|---|---|---|
| `OPENAI_API_KEY` | yes | — | Provider key (OpenAI or OpenRouter) |
...
| `ADTESTPRO_BASE_URL` | no | — | OpenAI-compatible endpoint, e.g. OpenRouter |
...
```mermaid
flowchart LR
    A["Brief + creative"] --> B["P1 · validate"]
    B --> C["Coverage personas<br/>(1–25)"]
    C --> D["Ad extraction<br/>(1 multimodal call)"]
    D --> E["Independent answers<br/>(1 call per persona)"]
    E --> F["Deterministic scores<br/>+ distributions"]
    F --> G["Theme synthesis"]
    G --> H["Evidence critic"]
    H --> I["Report + receipts"]
...
are pure Python. The aggregation step freezes scores before synthesis and the critic
run — the model can contextualize numbers but never move them.
...
Never commit `.env`. Provider keys are read from the environment only; the app never
logs keys, prompts, images, or persona profiles — run logs carry IDs, stage timings,
and token counts exclusively.
